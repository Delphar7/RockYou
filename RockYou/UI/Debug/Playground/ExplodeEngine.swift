...
// RockYou
//
// Engine for explode dome shatter effect.
// Fragments fly outward with gravity, spinning as they fall.
...
/// Engine for explode dome shatter effect.
@Observable
final class ExplodeEngine: PlaygroundEngine {
  ...
  var baseSpeed: Double = 0.15      // Outward velocity
  var spreadAngle: Double = 0.1     // Random spread
  var upwardBias: Double = 0.05     // Upward velocity component
  ...
  static let config: [PropertyConfig<ExplodeEngine>] = [
    .intField(\.fragmentCount, "Fragment Count"),
    .slider(\.domeRadius, "Dome Radius", 0.2...1.0, step: 0.05),
    .slider(\.baseSpeed, "Outward Speed", -2.0...2.0, step: 0.05),
    .slider(\.spreadAngle, "Spread Angle", 0.0...2.0, step: 0.05),
    .slider(\.upwardBias, "Upward Bias", -2.0...2.0, step: 0.05),
  ...
  func toShatterConfig() -> DomeShatterConfig {
    ...
    config.baseSpeed = Float(baseSpeed)
    config.spreadAngle = Float(spreadAngle)
    config.upwardBias = Float(upwardBias)
    return config
  }
